    ...
    <section class="mt-4 mb-5">
        ...
        <div class="row justify-content-center align-items-start gap-4">
            <div class="col-lg-6 bg-white border border-dark rounded p-4 note-zone" style="height: 400px; overflow: auto;">
                @if ($project->note)
                    <p style="text-align: justify; font-size:{{ $project->note->font_size }}px;">
                        {!! nl2br($project->note->body) !!}
                    </p>
                @else
    ...
    <script>
        $(document).ready(function() {
            window.createState = function() {
                ...
                $('.note-zone').append(
                    `<textarea class="form-control h-100 p-4" name="body" placeholder="Write your notes here..." onfocus="this.setSelectionRange(this.value.length,this.value.length)" oninput="textAreaChange(this.value)" @if($project->note) style="font-size:{{ $project->note->font_size }}px;" @endif>${noteText}</textarea>`
                )
            ...
            window.cancelState = function() {
                ...
                $('.note-zone').append(
                    `@if ($project->note) <p style="text-align: justify; font-size:{{ $project->note->font_size }}px;"> {!! nl2br($project->note->body) !!} </p> @else  <img class="w-50 mt-4 mb-3 d-block mx-auto" src="{{ asset('images/note-empty.svg') }}" alt=""> <h3 class="text-center">Empty</h3> @endif`
                )
            }
